...
import { Upload, Loader2, Download, Waves } from 'lucide-react'
...
const PRESETS = [
  {
    id: 'hiss',
    label: 'Hiss / tape noise',
    af: 'afftdn=nr=12:nf=-25:tn=1',
  },
  {
    id: 'hum',
    label: 'Hum (50/60 Hz)',
    af: 'highpass=f=80,equalizer=f=50:width_type=h:width=2:g=-18,equalizer=f=60:width_type=h:width=2:g=-18,equalizer=f=100:width_type=h:width=2:g=-10,equalizer=f=120:width_type=h:width=2:g=-10',
  },
  {
    id: 'gentle',
    label: 'Gentle denoise',
    af: 'afftdn=nr=6:nf=-20',
  },
  {
    id: 'strong',
    label: 'Strong denoise',
...
export default function NoiseReducer() {
  ...
  const [preset, setPreset] = useState<(typeof PRESETS)[number]['id']>('hiss')
  ...
  const process = async (f: File) => {
    ...
    try {
      ...
      const outName = `clean${outFmt.ext}`
      await ffmpeg.writeFile(inName, await fetchFile(f))
      setStatus('Reducing noise…')
      const code = await ffmpeg.exec(['-i', inName, '-af', p.af, ...outFmt.args, outName])
      if (code !== 0) throw new Error('Noise reduction failed. Try Gentle preset or MP3 output.')
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100 mb-3">Noise Reducer</h1>
        <p className="text-slate-500 dark:text-slate-400">
          Reduce hiss, hum, and background noise from recordings — privately in your browser. Best for constant noise, not talking under music.
        </p>
      </div>
      ...
      >
        {busy ? <Loader2 className="w-10 h-10 text-indigo-500 animate-spin" /> : <Waves className="w-10 h-10 text-slate-400" />}
        <div className="text-center">
          <p className="font-medium">{file ? file.name : 'Drop audio to denoise'}</p>
      ...
      {result && file && (
        <div className="mt-8 text-center space-y-3">
          ...
          <a href={result} download={`${baseName(file.name)}-denoised${AUDIO_OUTPUTS.find((o) => o.id === format)?.ext}`} className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 text-white font-medium">
