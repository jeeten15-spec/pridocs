...
export default function PngToJpg() {
  ...
  const handleFile = (f: File | null) => {
    if (!f) return
    if (!f.type.match(/image\/png/i) && !f.name.match(/\.png$/i)) {
    ...
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      >
        ...
        <div className="text-center">
          <p className="font-medium text-slate-700">{file ? file.name : 'Drop a PNG file here or click to browse'}</p>
          {file && <p className="text-sm text-slate-400 mt-1">{formatBytes(file.size)}</p>}
        </div>
        <input type="file" accept="image/png,.png" className="hidden" onChange={(e) => handleFile(e.target.files?.[0] || null)} />
      </label>
      ...
      {result && (
        <div className="mt-8 text-center">
          <div className="mb-4">
            <img src={result} alt="Converted JPG" className="max-h-96 mx-auto rounded-xl border border-slate-200 shadow-sm" />
          </div>
          <a
            href={result}
            download={file?.name.replace(/\.png$/i, '.jpg') || 'converted.jpg'}
